...
export class ExchangeComponent implements AfterViewInit, OnDestroy {
  ...
  exchanges = [
    ...
    },
    {
      name: 'EtherDelta',
      url:
        'https://etherdelta.com/#0x1d462414fe14cf489c7a21cac78509f4bf8cd7c0-ETH',
    },
    {
      name: 'Coss.io',
  ...
  async ngAfterViewInit() {
    this.web3Subscription = this.ethService.web3Status$.subscribe(
      (status: Web3LoadingStatus) => {
        ...
        if (status === Web3LoadingStatus.complete) {
          ...
          this.accountSubscription = this.ethService.account$.subscribe(
            async (acc: string) => {
              if (acc !== undefined) {
                this.account = acc
