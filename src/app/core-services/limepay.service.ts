...
export class LimepayService {
  ...
  constructor(
    ...
    LimePayWeb.connect(LimePayWeb.Environment[environment.limepay.env])
      .then(limepay => {
  ...
  async getEnterEscrowTransactions(jobId): Promise<any> {
    try {
      const res = await this.http
        .get(
          `${apiUrl}/auth/enter-escrow-tx?jobId=${jobId}`,
  ...
  async getPaymentStatus(paymentId): Promise<any> {
    try {
      const res = await this.http
        .get(
          `${apiUrl}/auth/get-payment-status?paymentId=${paymentId}`,
          await this.getOptions()
        )
        .take(1)
        .toPromise()
  ...
  async monitorPayment(paymentId, jobId): Promise<any> {
    try {
      const res = await this.http
        .get(
          `${apiUrl}/auth/monitor?paymentId=${paymentId}&jobId=${jobId}`,
          await this.getOptions()
        )
        .take(1)
        .toPromise()
  ...
  async initFiatPayment(jobId, providerId): Promise<any> {
    try {
      ...
      const res = await this.http
        .post(
          `${apiUrl}/auth/initFiatPayment`,
          { jobId, providerEthAddress: provider.ethAddress },
          await this.getOptions()
        )
        .take(1)
        .toPromise()
  ...
  async initRelayedPayment(jobId, userId): Promise<any> {
    try {
      const res = await this.http
        .post(
          `${apiUrl}/auth/initRelayedPayment`,
          { jobId, userId },
          await this.getOptions()
        )
        .take(1)
  ...
  async createShopper() {
    try {
      const res = await this.http
        .post(`${apiUrl}/auth/createShopper`, {}, await this.getOptions())
        .take(1)
        .toPromise()
      console.log(res)
      return Promise.resolve(res.json())
  ...
  async getShopper() {
    try {
      const res = await this.http
        .get(`${apiUrl}/auth/getShopper`, await this.getOptions())
        .take(1)
        .toPromise()
      console.log(res)
      return Promise.resolve(res.json())
  ...
  async getWalletToken() {
    try {
      const res = await this.http
        .get(`${apiUrl}/auth/getWalletToken`, await this.getOptions())
        .take(1)
        .toPromise()
  ...
  getTransactionsByJob(jobId: string) {
    return this.afs
      .collection<any>('limepay-payments', ref =>
