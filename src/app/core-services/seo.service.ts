...
export class SeoService {
  ...
  updateAllSeoProperties(type: string, title: string, description: string, url: string, avatar: any) {
    let cwTitle = this.siteName + ' - ' + title;
    let cwUrl = window.location.protocol+'//'+window.location.host+'/profile/'+url;
    let cwImg = null;
    if (avatar && avatar.uri && avatar.uri.trim()) cwImg = avatar.uri.trim();
