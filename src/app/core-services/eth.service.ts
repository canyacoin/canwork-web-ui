...
export class EthService implements OnDestroy {
  ...
  public web3Status = new BehaviorSubject<Web3LoadingStatus>(
    Web3LoadingStatus.loading
  )
  ...
  constructor(protected http: Http) {
    if (window.ethereum || window.web3) {
      ...
      this.web3js = new Web3(
        new Web3.providers.HttpProvider(environment.backupWeb3Provider)
      )
      this.web3Status.next(Web3LoadingStatus.noMetaMask)
    }

    this.canyaContract = this.createContractInstance(
      canyaAbi,
      environment.contracts.canYaCoin
    )
  }
  ...
  private checkAccountMetaMask() {
    this.web3js.eth.getAccounts().then((accs: string[]) => {
      if (accs[0] !== this.account.value) {
        console.log(
          'Web3Service: new account found: ' + JSON.stringify(accs[0])
        )
        this.account.next(accs[0])
        if (accs[0] !== undefined) {
          // && (this.web3Status.value !== Web3LoadingStatus.complete)
          this.ownerAccount = accs[0]
          this.web3Status.next(Web3LoadingStatus.complete)
          return
        }
  ...
  async getEthBalanceAsync(
    userAddress: string = this.getOwnerAccount()
  ): Promise<string> {
    if (userAddress) {
  ...
  createContractInstance(
    abi,
    address,
    useDefaultWeb3Provider: boolean = false
  ) {
    console.log(
      'createContractInstance: ',
      abi,
      address,
      useDefaultWeb3Provider
    )
  ...
  async payWithEther(amount: number, to: string): Promise<any> {
    ...
    return new Promise((resolve, reject) => {
      this.web3js.eth.sendTransaction(
        ...
        async (err, txHash) =>
          this.resolveTransaction(err, from, txHash, resolve, reject)
      )
    })
  }

  async payWithErc20Token(
    abi,
    recipient: string,
    amount: number,
    address,
    ...
    return new Promise(async (resolve, reject) => {
      ...
      tx.send({ from, gas: txGas, gasPrice: txGasPrice }, async (err, txHash) =>
        this.resolveTransaction(err, from, txHash, resolve, reject)
      )
    })
  }
      ...
      try {
        ...
        receipt.status =
          typeof receipt.status === 'boolean'
            ? receipt.status
            : this.web3js.utils.hexToNumber(receipt.status)
        resolve(receipt)
      } catch (e) {
        reject(e)
      }
  ...
  async getAllowance(owner: string, spender: string): Promise<number> {
    try {
      const allowance = await this.canyaContract.methods
  ...
  async getCanYaBalance(
    userAddress: string = this.getOwnerAccount()
  ): Promise<string> {
    try {
      if (userAddress) {
        const balance = await this.canyaContract.methods
          .balanceOf(userAddress)
          .call()
        console.log('CAN balance: ', balance)
        const t = this.web3js.utils.toBN(balance)
        return Promise.resolve(
          t.div(this.web3js.utils.toBN(10 ** this.canyaDecimals)).toString()
        )
      }
  ...
  authoriseCANPayment(
    torecipient,
    amount,
    from: string = this.getOwnerAccount(),
    onTxHash: Function = null
  ): Promise<any> {
    console.log('EthService: authoriseCANPayment: ', from, torecipient, amount)
    return new Promise(async (resolve, reject) => {
      const tx = await this.canyaContract.methods.approve(
        torecipient,
        this.amountToCANTokens(amount)
      )
      const gas = await tx.estimateGas()
      const gasPrice = await this.getDefaultGasPriceGwei()
      tx.send({ from, gas, gasPrice }, async (err, txHash) =>
        this.resolveTransaction(err, from, txHash, resolve, reject, onTxHash)
      )
    })
  }
  ...
  payWithCAN(
    torecipient,
    amount,
    from = this.getOwnerAccount(),
    onTxHash: Function = null
  ): Promise<any> {
    console.log('EthService: payWithCAN: ', from, torecipient, amount)
    return new Promise(async (resolve, reject) => {
      const tx = await this.canyaContract.methods.transfer(
        torecipient,
        this.amountToCANTokens(amount)
      )
      let gas, gasPrice
  ...
  async getCanToUsd(amountOfCan: number = 1): Promise<number> {
    try {
      const priceOracle = this.createContractInstance(
        priceOracleAbi,
        environment.contracts.priceOracle,
        this.web3Status.value !== Web3LoadingStatus.complete
      )
      const tokenValueInDai = await priceOracle.methods
        .getTokenToDai((amountOfCan * 10 ** this.canyaDecimals).toString())
        .call()
  ...
  async getUsdToCan(amountOfUsd: number = 1): Promise<number> {
    try {
      ...
      const daiValueInToken = await priceOracle.methods
        .getDaiToToken(dai)
        .call()
