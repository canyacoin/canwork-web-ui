...
export class CanexQRComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    try {
      this.statusInterval = setInterval(() => {
        this.canexService.checkStatus(this.formData.key).subscribe(
          activity => {
            try {
  ...
  metamask() {
    if (this.formData.currency === 'ETH') {
      this.ethService.payWithEther(
        this.formData.eth,
        this.canexService.environment.backendEthAddress
      )
    } else {
      this.gasSub = this.canexService.getGasPrice().subscribe(activity => {
        this.ethService.payWithERC20(
          this.formData.eth,
          this.canexService.environment.backendEthAddress,
          this.formData.erc20token,
          this.formData.erc20tokenDecimal,
