...
export class CanexService {
  ...
  get environment(): CanexConfig {
    if (environment.contracts.network !== NetworkType.main) {
      return {
        ...
        bancor:
          'https://api.bancor.network/0.1/currencies/CAN/ticker?fromCurrencyCode=',
  ...
  save(data: any): Observable<any> {
    return this.http.post(
      this.environment.backendUrl + '/api/saveTransaction',
      data
    )
  }
  ...
  getByAddress(address: string): Observable<any> {
    return this.http.get(
      this.environment.backendUrl + '/api/getbyaddress/' + address
    )
  }
  ...
  sentMailStaging(data: any): Observable<any> {
    return this.http.post(
      this.environment.backendUrl + '/api/sendStagingMail',
      data
    )
  }
