...
export class CanexOrderStatusComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    this.currency = 'CAN'
    this.orderSub = this.canexService
      .getOrder(this.route.snapshot.params.id)
      .subscribe(
        activity => {
          this.orderData = activity
          this.etherUrl =
            this.canexService.environment.etherscan + this.orderData.hash
  ...
  getOrder() {
    ...
    this.orderSub = this.canexService.getOrder(this.orderid).subscribe(
      activity => {
        this.orderData = activity
        this.etherUrl =
          this.canexService.environment.etherscan + this.orderData.hash
