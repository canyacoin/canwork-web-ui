...
import {
  ...
  FormDataService,
  Personal,
} from '../services/formData.service'
...
export class CanexPaymentOptionsComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.canexService
      ...
      .take(1)
      .subscribe(data => {
        this.formData.eth = Number(
          (this.formData.amount * data.json().data.price).toFixed(6)
        )
      })
    this.canexService
      .getData('USD')
  ...
  selectCurrency(currency) {
    ...
    if (currency === 'ETH') {
      ...
      this.dataSub = this.canexService.getData('BNB').subscribe(data => {
      ...
      this.dataSub = this.canexService.getData('DAI').subscribe(data => {
        const price = data.json().data.price * +this.formData.amount
        this.formData.eth = Number(price.toFixed(6))
  ...
  next() {
    if (this.erc20) {
      ...
      this.validData &&
      ((this.bnb && this.gotBnbPrice) || (this.dai && this.gotDaiPrice))
    ) {
