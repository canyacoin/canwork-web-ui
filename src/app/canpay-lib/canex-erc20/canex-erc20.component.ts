...
export class CanexERC20Component implements OnInit, OnDestroy {
  ...
  search(val) {
    ...
    if (val) {
      this.filteredTokens = this.tokens.filter(
        c =>
          c.name.toUpperCase().match(val.toUpperCase()) ||
          c.symbol.toUpperCase().match(val.toUpperCase())
      )
    } else {
  ...
  selectCurrency(form, key) {
    ...
    this.dataSub = this.canexService.getData(form.symbol).subscribe(data => {
  ...
  get nextButtonDisabled(): boolean {
    return (
      !this.status || this.formData.currency === 'erc20' || !this.formData.eth
    )
  }
