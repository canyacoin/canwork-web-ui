...
export class WithDockComponent implements OnInit, OnDestroy {
  ...
  async init(code: string) {
    ...
    this.dockAuthSub = this.dockIoService.authCollection
      ...
      .subscribe(action => {
        action.some(({ payload, type }) => {
          ...
          const isFromDockContext =
            data['redirectURIAuthCode'] && data['userID'] ? true : false
          const remoteAuthCodeMatchesLocalCode =
            data['redirectURIAuthCode'] === code
