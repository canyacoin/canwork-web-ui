 ...
 */
exports.generateAuthPinCode = functions.https.onRequest(
  async (request, response) => {
    cors(request, response, async () => {
      if (request.method !== 'POST') {
        return response
      ...
      if (typeof user !== undefined) {
        ...
        try {
          await db
            .collection('users')
            .doc(user.address)
            .update({ ethereumLogin: { pin, expiry } })
        } catch (e) {
          return response
            .status(500)
            .type('application/json')
        ...
        console.log('+ generated mobile/ethereum login pin', {
          email: user.email,
        ...
        return response
          ...
          .type('application/json')
          .send({
            message: 'Secure pin generated and delivered',
            email: user.email,
          })
      } else {
        return response
 ...
 */
exports.ethereumAuthViaPinCode = functions.https.onRequest(
  async (request, response) => {
    cors(request, response, async () => {
      if (request.method !== 'POST') {
        return response
      ...
      if (typeof user !== undefined && user.ethereumLogin !== undefined) {
        ...
        if (user.ethereumLogin.pin === pinCode) {
          ...
          if (now <= user.ethereumLogin.expiry) {
            ...
            try {
              ...
              console.error(
                '+ unable to generate auth token for request.body:',
                request.body
              )
        ...
        console.log(
          '+ unable to locate user object in firestore, request.body was:',
          request.body
        )
        return response
          .status(403)
 ...
 */
exports.getFirebaseTokenForDockIOAuth = functions.https.onRequest(
  async (request, response) => {
    cors(request, response, async () => {
      if (request.method !== 'POST') {
        return response
...
exports.initVerifiedUserField = functions.https.onRequest(
  async (request, response) => {
    ...
    usersnaps.forEach(async doc => {
      const user =
        doc.data()(user.verified !== true) &&
        updateUser(doc.id).catch(err => console.error(err))
    })
